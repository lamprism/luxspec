 ...
 * @author RollW
 */
public class PasswordAuthenticator implements Authenticator<UsernamePasswordCredential> {
    private static final CredentialType<UsernamePasswordCredential> CREDENTIAL_TYPE = CredentialType.of(
            "username-password",
     ...
     */
    @Override
    public CredentialType<UsernamePasswordCredential> getCredentialType() {
     ...
     */
    @Override
    public Authentication authenticate(UsernamePasswordCredential credentials) {
        UsernamePasswordCredential nonNullCredential = Objects.requireNonNull(credentials, "credentials");
        User user = findUser(nonNullCredential.getUsername());
        UserSubject subject = UserSubjectResolver.requireActive(user);
        EncodedPassword encodedPassword = findPassword(user.id());
        if (!passwordScheme.verify(nonNullCredential.getRawPassword(), encodedPassword)) {
            throw invalidCredential();
        }
        upgradeIfNeeded(user.id(), encodedPassword, nonNullCredential.getRawPassword());
    ...
    private static AuthenticationException invalidCredential() {
