 ...
 *
 * <p>This is an advanced JPA mapping SPI. Standard executor and factory construction uses direct
 * same-name attribute mapping internally.</p>
 *
 * @param <T> the persistence entity type
 * @author RollW
 */
public interface JpaQueryFieldResolver<T> {
    /**
     ...
     * {@link Root#get(String)}. It does not inspect entity fields or methods through Java reflection.</p>
     *
     * @param <T> the persistence entity type
     * @return a resolver for direct same-name attributes
     */
    static <T> JpaQueryFieldResolver<T> byAttributeName() {
     ...
     */
    static <T> JpaQueryFieldResolver<T> byAttributeName(JpaFieldNamingStrategy namingStrategy) {
        return new DefaultJpaQueryFieldResolver<>(Objects.requireNonNull(namingStrategy, "namingStrategy"));
    }
