 ...
 * @author RollW
 */
public final class UserEmailChangedAuditTranslator implements AuditEventTranslator<UserEmailChangedEvent> {
    @Override
    public AuditEntryContent translate(AuditEnvelope<UserEmailChangedEvent> envelope) {
        ...
        AuditFieldSet fields = AuditFieldSet.builder()
                ...
                .put(LuxspecAuditFields.USER_EMAIL_PRESENT, event.isEmailPresent())
                .build();
        return UserAuditTranslationSupport.userEntry("user.email.change", event.getUserId(), fields);
    }
}
