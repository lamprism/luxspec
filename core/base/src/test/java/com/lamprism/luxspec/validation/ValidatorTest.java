...
class ValidatorTest {
    ...
    void composesTextRulesWithoutNormalizingTheValue() {
        Validator<String> validator = Validator
                .nonBlank("name")
                .and(Validator.noWhitespace("name"))
                .and(Validator.noControlCharacters("name"));
    ...
    void validatesListElementsWithTheSameRuleContract() {
        Validator<List<String>> validator = Validator.elements(
                Validator.nonBlank("element")
        );
    ...
    void validatesLengthAndMembershipRules() {
        Validator<String> length = Validator.maxLength("name", 4);
        Validator<String> membership = Validator.oneOf("kind", Set.of("user", "service"));
    ...
    void rejectsNullValuesBeforeApplyingRules() {
        Validator<String> validator = Validator.nonBlank("name");
    ...
    void rejectsInvalidRuleDefinitions() {
        assertThrows(IllegalArgumentException.class, () -> Validator.nonBlank(" "));
        assertThrows(IllegalArgumentException.class, () -> Validator.maxLength("name", 0));
        assertThrows(IllegalArgumentException.class, () -> Validator.oneOf("kind", Set.of()));
    }
}
