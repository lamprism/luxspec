...
class CaffeineCache<K, V> implements Cache<K, V> {
    ...
    public @Nullable V getIfPresent(K key) {
        return delegate.getIfPresent(Objects.requireNonNull(key, "key"));
    }
    ...
    public V get(K key, Function<? super K, ? extends V> loader) {
        ...
        while (true) {
            ...
            V loadedValue;
            try {
                loadedValue = load(nonNullKey, nonNullLoader);
    ...
    private V load(K key, Function<? super K, ? extends V> loader) {
        return Objects.requireNonNull(loader.apply(key), "loaded value");
    }
    ...
    public void invalidateAll(Predicate<? super K> predicate) {
        ...
        synchronized (stateLock) {
            for (Map.Entry<K, LoadState<V>> entry : inFlightLoads.entrySet()) {
                if (nonNullPredicate.test(entry.getKey())) {
                    entry.getValue().invalidated = true;
                }
            }
            delegate.asMap().keySet().removeIf(nonNullPredicate);
        }
    }
