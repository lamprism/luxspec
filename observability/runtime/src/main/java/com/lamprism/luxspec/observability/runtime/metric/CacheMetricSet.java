...
import com.lamprism.luxspec.cache.CacheStatisticsSnapshot;
...
public class CacheMetricSet implements MetricSet {
    ...
    public void register(MetricRegistry registry) {
        ...
        FunctionCounterSpec<Object> hits = counter(
                ...
                source -> snapshot(source).getHitCount(),
                cacheDimension,
                "requests"
        );
        FunctionCounterSpec<Object> misses = counter(
                ...
                source -> snapshot(source).getMissCount(),
                cacheDimension,
                "requests"
        );
        FunctionCounterSpec<Object> loads = counter(
                ...
                source -> snapshot(source).getLoadSuccessCount(),
                cacheDimension,
                "loads"
        );
        FunctionCounterSpec<Object> loadFailures = counter(
                "cache.load.failure",
                "Failed cache loads",
                source -> snapshot(source).getLoadFailureCount(),
                cacheDimension,
                "loads"
        );
        FunctionCounterSpec<Object> loadDuration = counter(
                ...
                source -> snapshot(source).getLoadDurationNanos(),
                cacheDimension,
                "nanoseconds"
        );
        FunctionCounterSpec<Object> evictions = counter(
                ...
                source -> snapshot(source).getEvictionCount(),
                cacheDimension,
                "entries"
        );
        GaugeSpec<Object> size = GaugeSpec
                ...
                .reader(source -> snapshot(source).getCurrentSize())
                .build();
    ...
    private static CacheStatisticsSnapshot snapshot(Object source) {
        CacheStatisticsSource statisticsSource = (CacheStatisticsSource) source;
        return statisticsSource.snapshot();
    }
