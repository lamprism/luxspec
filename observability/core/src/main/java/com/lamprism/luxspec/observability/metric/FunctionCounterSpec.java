...
/**
 * Declares a value-backed cumulative function counter.
 *
 * @author RollW
 */
public final class FunctionCounterSpec<S> extends AbstractMetricSpec<FunctionCounter> {
    private final Class<S> sourceType;
    private final NumberReader<? super S> reader;

    private FunctionCounterSpec(Builder<S> builder) {
    ...
    public MetricBinding<FunctionCounter> bind(MetricDimensionSet dimensions, S source) {
        return bindValueSource(dimensions, source);
    }

    @Override
    public boolean equals(Object other) {
        if (!(other instanceof FunctionCounterSpec<?> spec) || !super.equals(other)) {
            return false;
        }
    ...
    /**
     * Builds a function counter specification.
     */
    public static final class Builder<S> extends BuilderSupport<FunctionCounter, Builder<S>> {
        private final Class<S> sourceType;
        private NumberReader<? super S> reader;

        private Builder(String name, Class<S> sourceType) {
            super(name, MetricKind.FUNCTION_COUNTER);
        ...
        public FunctionCounterSpec<S> build() {
            if (reader == null) {
                throw new IllegalStateException("A function counter reader is required");
            }
            return new FunctionCounterSpec<>(this);
        }
    }
}
